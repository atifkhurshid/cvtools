...
from ..metrics import compute_roc, compute_multilabel_roc
from ..metrics import multilabel_auc, multilabel_accuracy
...
def evaluate_multilabel_classification(
    ...
    """
    Evaluate multilabel classification model performance.

    Parameters
    -----------
    y_true : np.ndarray of shape (n_samples, n_classes)
        True one-hot labels of the data.
    y_pred : np.ndarray of shape (n_samples, n_classes)
        Predicted one-hot labels by the model.
    outputs : np.ndarray of shape (n_samples, n_classes), optional
        Model output logits for each class. Required if roc is True.
    class_names : list, optional
        Names of the classes. If None, uses integer labels.
    roc : bool, optional
    ...
    **kwargs : dict
    ...
    print("Multilabel Accuracy: {:.4f}".format(accuracy))
    print("Multilabel AUC: {:.4f}".format(auc_score))

    if roc:
        fpr, tpr, _, auc_scores = compute_multilabel_roc(
            y_true,
            outputs,
            classes = class_names,
        )
        if len(class_names) <= 20:
            roc_titles = [*class_names, "micro", "macro", "weighted"]
        else:
            roc_titles = [ "micro", "macro", "weighted"]
    ...
    if report:
        ...
        report["multilabel_accuracy"] = accuracy
        report["multilabel_auc"] = auc_score
