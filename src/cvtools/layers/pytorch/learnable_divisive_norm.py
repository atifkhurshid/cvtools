...
class LearnableDivisiveNorm(nn.Module):
    ...
    def __init__(
        ...
        self.avgpool = nn.AvgPool2d(kernel_size=pool_size, stride=1, padding=pool_size//2)
        self.weights = nn.Parameter(
            torch.normal(mean=1, std=0.01, size=(1, in_channels, surround_size, surround_size)),
            requires_grad=True
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        divisor = self.avgpool(x)

        weights = torch.abs(self.weights)
        divisor = F.conv2d(divisor, weights, padding=weights.shape[2]//2)
